                ...
                <div class="p-6 text-gray-900">
                    ...
                    <form action="{{ route('catalogo.update', $catalogoId) }}" method="POST" class="space-y-4">
                        ...
                        <div>
                            ...
                            <input type="text" name="nombre" id="nombre" value="{{ old('nombre', 'Mesa de comedor') }}" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:border-indigo-500 focus:ring-indigo-500" required>
                        </div>

                        <div>
                            <label for="material" class="block text-sm font-medium text-indigo-500">Material</label>
                            <input type="text" name="material" id="material" value="{{ old('material', 'Madera') }}" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                        </div>

                        <div>
                            <label for="descripcion" class="block text-sm font-medium text-indigo-500">Descripción</label>
                            <textarea name="descripcion" id="descripcion" rows="3" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:border-indigo-500 focus:ring-indigo-500">{{ old('descripcion', 'Mesa rectangular para comedor') }}</textarea>
                        </div>

                        <div class="grid grid-cols-2 gap-4">
                            <div>
                                <label for="precio_unitario" class="block text-sm font-medium text-indigo-500">Precio unitario</label>
                                <input type="number" step="0.01" name="precio_unitario" id="precio_unitario" value="{{ old('precio_unitario', '4500.00') }}" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:border-indigo-500 focus:ring-indigo-500" required>
                            </div>

                            <div>
                                <label for="stock" class="block text-sm font-medium text-indigo-500">Stock</label>
                                <input type="number" name="stock" id="stock" value="{{ old('stock', 10) }}" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:border-indigo-500 focus:ring-indigo-500" required>
                            </div>
                        </div>
