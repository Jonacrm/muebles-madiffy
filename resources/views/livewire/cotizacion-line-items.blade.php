                        ...
                        @foreach ($lineas as $index => $linea)
                            <tr wire:key="cotizacion-linea-{{ $index }}-{{ $linea['product_id'] ?? 'nueva' }}">
                                ...
                                <td class="py-2 px-4 border-b text-right text-sm text-gray-600">
                                    <input type="number" name="items[{{ $index }}][quantity]" wire:model.live="lineas.{{ $index }}.quantity" value="{{ $linea['quantity'] }}" min="1" class="w-20 rounded-md border-gray-300 text-right text-sm shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                                    @error("items.$index.quantity")
                                        <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
                                    @enderror
                                </td>
                                <td class="py-2 px-4 border-b text-right text-sm text-gray-600">
                                    <input type="number" name="items[{{ $index }}][unit_price]" wire:model.live="lineas.{{ $index }}.unit_price" value="{{ $linea['unit_price'] }}" step="0.01" min="0" class="w-32 rounded-md border-gray-300 text-right text-sm shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                                    @error("items.$index.unit_price")
                                        <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
                                    @enderror
                                </td>
                                <td class="py-2 px-4 border-b text-right text-sm text-gray-600">
                                    <input type="number" name="items[{{ $index }}][line_discount]" wire:model.live="lineas.{{ $index }}.line_discount" value="{{ $linea['line_discount'] }}" step="0.01" min="0" class="w-32 rounded-md border-gray-300 text-right text-sm shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
    ...
    <div class="grid gap-6 lg:grid-cols-3">
        <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg lg:col-span-2">
            <div class="p-6">
                ...
                <textarea name="notas" id="notas" rows="4" wire:model.live.debounce.500ms="notas" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">{{ $notas }}</textarea>
            </div>
        </div>
